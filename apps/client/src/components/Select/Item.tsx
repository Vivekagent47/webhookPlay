...
const Item: React.FC<ItemProps> = ({ item, primaryColor }) => {
  ...
  const isSelected = useMemo(() => {
    return value !== null && !Array.isArray(value) && value.id === item.id;
  }, [item.id, value]);
  ...
  const bgHoverColor = useMemo(() => {
    if (COLORS.includes(primaryColor)) {
      return THEME_DATA.bgHover[
        primaryColor as keyof typeof THEME_DATA.bgHover
      ];
    }
  ...
  return (
    <>
      {formatOptionLabel ? (
        ...
        <>
          {item.disabled ? (
            <DisabledItem>{item.name}</DisabledItem>
          ) : (
            <li
