...
const Select: React.FC<SelectProps> = ({
  ...
  const removeItem = useCallback(
    (e: React.MouseEvent<HTMLDivElement>, item: Option) => {
      if (isMultiple && Array.isArray(value) && value.length) {
        e.stopPropagation();
        const result = value.filter((current) => item.id !== current.id);
  ...
  const getSelectClass = useCallback(() => {
    ...
    const baseClass =
      "flex text-sm text-slate-500 border border-slate-300 rounded shadow-sm transition-all duration-300 focus:outline-none";
    const defaultClass = `${baseClass} ${
      isDisabled
        ? "bg-slate-200"
        : `bg-slate-50 hover:border-slate-400 ${borderFocus} focus:ring ${ringColor}`
    }`;
  ...
  const getTagItemClass = useCallback(
    (item: Option) => {
      const baseClasse = "bg-slate-200 border rounded-sm flex space-x-1";
      const disabledClass = isDisabled ? "border-slate-500 px-1" : "pl-1";
        ...
        >
                      ...
                      <p
                        className={
                          classNames?.tagItemText
                            ? classNames.tagItemText
                            : "text-slate-600 truncate cursor-default select-none"
                        }
                      >
                        {item.name}
                      </p>
          ...
          <div className="flex flex-none items-center py-1.5">
            ...
            <div className="h-full">
              <span className="w-px h-full inline-block text-slate-50 bg-slate-300 text-opacity-0" />
            </div>
            ...
            <div className="px-1.5">
              <ChevronIcon
                className={`transition duration-300 w-6 h-6 p-0.5${
                  open
                    ? " transform rotate-90 text-slate-500"
                    : " text-slate-300"
                }`}
              />
            </div>
        ...
        {open && !isDisabled && (
          <div
            ...
            className={
              classNames?.menu
                ? classNames.menu
                : "absolute z-99 w-full bg-slate-50 shadow-lg border border-slate-200 rounded py-1 mt-1.5 text-sm text-slate-700"
            }
          >
            {isSearchable && (
              <SearchInput
